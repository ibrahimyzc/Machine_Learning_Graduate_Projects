...
#### SOLUTION
The code chunks below show that "Xmat_quad_small" has 5 rows and 3 colums. Also "Xmat_quad_big" has 30 rows and 3 colums.
```{r, solution_for_1c_a, eval=TRUE}
dim(Xmat_quad_small)
dim(Xmat_quad_big)
```
...
sigma_true = 1
covmat_quad_small_noprior <- sigma_true^2*solve(t(Xmat_quad_small[drop=FALSE]) %*% Xmat_quad_small[drop=FALSE])
```
...
Each posterior covariance matrix has 3 rows and 3 colums as it can be seen in the code chunks below.

```{r, solution_for_1e, eval=TRUE}
...
The code chunks below shows that the posterior standard deviation on the quadratic feature for the small data case is 1.3021277, and the posterior standard deviation on the quadratic feature for the big data case is 0.1566305.

```{r, solution_for_1f_a, eval=TRUE}
...
postmeans_quad_small_noprior <- (solve(t(Xmat_quad_small[drop=FALSE]) %*% Xmat_quad_small[drop=FALSE])%*%t(Xmat_quad_small[drop=FALSE]))%*%y_quad_small_col
...
postmeans_quad_big_noprior <- (solve(t(Xmat_quad_big[drop=FALSE]) %*% Xmat_quad_big[drop=FALSE])%*%t(Xmat_quad_big[drop=FALSE]))%*%y_quad_big_col
postmeans_quad_big_noprior
```


In both cases posterior means does not change if $\sigma$ is changed because the noise $\sigma$ does NOT explicitly impact the MLEs.
...
covmat_quad_small_inform <-   solve(solve(B0_inform)+ (t(Xmat_quad_small[drop=FALSE])%*%Xmat_quad_small[drop=FALSE])/sigma_true^2)
```


```{r, solution_for_2b_b, eval=TRUE}
### posterior covariance matrix INFORMATIVE prior and BIG data
covmat_quad_big_inform  <-   solve(solve(B0_inform)+ (t(Xmat_quad_big[drop=FALSE])%*%Xmat_quad_big[drop=FALSE])/sigma_true^2)
```
...
covmat_quad_small_vague <- solve(solve(B0_vague)+ (t(Xmat_quad_small[drop=FALSE])%*%Xmat_quad_small[drop=FALSE])/sigma_true^2)
```


```{r, solution_for_2c_b, eval=TRUE}
### posterior covariance matrix VAGUE prior and BIG data
covmat_quad_big_vague <- solve(solve(B0_vague)+ (t(Xmat_quad_big[drop=FALSE])%*%Xmat_quad_big[drop=FALSE])/sigma_true^2)
```
...
ggplot(data = post_samples_quad, aes(x = beta_02)) +
  geom_histogram(binwidth = (max(post_samples_quad$beta_02)-min(post_samples_quad$beta_02))/55, fill = "blue", color = "black") +
  labs(title = "Posterior Histogram for beta_02", x= "beta_02", y= "Frequency")
```

The posterior 95% uncertainty interval on $\beta_2$ is: [`r mean(post_samples_quad$beta_02)-2*sd(post_samples_quad$beta_02)`,`r mean(post_samples_quad$beta_02)+2*sd(post_samples_quad$beta_02)`].

The probability that $\beta_2$ is greater than zero is 0 as calculated in the code chunk below.
...
Overfitting typically occurs when the model performs very well on the training data but poorly on the test data. Also, if there is a clear trend where training MAE continues to decrease while test MAE starts to rise, it's an indication of overfitting. So, based of the last plot above we can conclude that, for DOF greater than 15, overfitting occurs.
...
The 7 DOF spline has the highest weight, considered to be the most plausible.
The 9 DOF spline has the second highest weight.
