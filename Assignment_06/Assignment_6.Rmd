...
Firstly, we obtained the unnormalized log posterior on $\mu$ as:

$$
\log\left[p(\mu\mid\mathbf{x},\sigma)\right] \propto -\frac{1}{2\sigma^2}\sum_{n=1}^N \left\{(x_n-\mu)^2\right\}-\frac{1}{2\tau^2_0}(\mu-\mu_0)^2
$$
Then, in HW5, we obtained its first derivatiove as:

$$
\frac{d}{d \mu}\left\{ -\frac{1}{2\sigma^2}\sum_{n=1}^N \left\{(x_n-\mu)^2\right\}-\frac{1}{2\tau^2_0}(\mu-\mu_0)^2\right\}=\frac{1}{\sigma^2}\sum_{n=1}^N(x_n-\mu)-\frac{1}{\tau^2_0}(\mu-\mu_0)=\frac{1}{\sigma^2}\left(\sum_{n=1}^N(x_n)-\sum_{n=1}^N(\mu)\right)-\frac{1}{\tau^2_0}(\mu-\mu_0)
$$
$$
=\frac{1}{\sigma^2}\left(\bar{x}N-N\mu\right)-\frac{1}{\tau^2_0}(\mu-\mu_0)
$$
$$
=\mu\left(-\frac{N}{\sigma^2}-\frac{1}{\tau^2_0}\right)+\frac{N}{\sigma^2}\bar{x}+\frac{\mu_0}{\tau^2_0}
$$
...
$$
\frac{d^2}{d \mu^2}\left\{ -\frac{1}{2\sigma^2}\sum_{n=1}^N \left\{(x_n-\mu)^2\right\}-\frac{1}{2\tau^2_0}(\mu-\mu_0)^2\right\}=-\frac{N}{\sigma^2}-\frac{1}{\tau^2_0}
$$
...
We obtained that the 2nd derivative equals $-\frac{N}{\sigma^2}-\frac{1}{\tau^2_0}$. It is negative for all values of $\mu$. So, it is negative at the critical point where the 1st derivative was 0. From Calculus, we know that if the 2nd derivative evaluated at a critical point gives a negative value, then the critical point is a maximum point of the function (this is called the 2nd derivative test). Thus, the result follows from the 2nd derivative test.
...
The approximate posterior mean $m_N$ is expressed as:
$$
m_{N}=\frac{\frac{N}{\sigma^2}\bar{x}+\frac{\mu_0}{\tau^2_0}}{\frac{N}{\sigma^2}+\frac{1}{\tau^2_0}}
$$
...
$$
s_{N}=\frac{1}{\sqrt{\frac{N}{\sigma^2}+\frac{1}{\tau^2_0}}}
$$
...
The un-normalized joint posterior between the unknown mean, $\mu$, and the transformed noise, $\varphi$, via the probability change-of-variables formula is:

$$ 
p \left( \mu, \sigma \mid \mathbf{x} \right) \propto \prod_{n=1}^{N} \left( \mathrm{normal} \left(x_n \mid \mu, g^{-1} \left( \varphi \right) \right) \right) \times \mathrm{normal}\left(\mu \mid \mu_0, \tau_0\right) \times \mathrm{Exp}\left(g^{-1} \left( \varphi \right) \mid \lambda\right)\cdot\left| \frac{d}{d \varphi}\left(g^{-1} \left( \varphi \right)\right)\right|
$$
...
The inverse transformation function is: $g^{-1} \left( \varphi \right)=\exp(\varphi)$.

The log of the derivative adjustment is: $\log\left| \frac{d}{d \varphi}\left(g^{-1} \left( \varphi \right)\right)\right|=\log(\exp(\varphi))=\varphi$.
...
3) Posterior standard deviation on $\mu$ is approximately $\sqrt{0.20696580}$=`r sqrt(0.20696580)`.

4) Posterior standard deviation on $\varphi$ is approximately $\sqrt{0.05496740}$=`r sqrt(0.05496740)`.

5) Posterior correlation coefficient between $\mu$ and $\varphi$ is $\frac{0.01291656}{0.4549349 \,*\, 0.2344513}$=`r 0.01291656/(0.4549349*0.2344513)`.
...
In problem 1a), we obtained that the sample average is 10.21831. In problem 4e), we obtained that posterior means on $\mu$ is 10.855893. Also, using c.o.v. we can calculate posterior means on $\sigma$ as $\exp(1.090389)$=`r exp(1.090389)`. 
...
ggplot(data = post_samples, aes(x = mu)) +
  geom_histogram(binwidth = (max(post_samples$mu)-min(post_samples$mu))/55, fill = "blue", color = "black") +
  labs(title = "Posterior Histogram for mu", x= "mu", y= "Frequency")

ggplot(data = post_samples, aes(x = sigma)) +
  geom_histogram(binwidth = (max(post_samples$sigma)-min(post_samples$sigma))/55, fill = "green", color = "black") +
