...
Let $x_n$ represent a single observation and $\mu$ represent the event probability. Then we can express the following:
...
Here are the values associated with the 1st, 2nd, 3rd, 4th, 5th, and 6th observations of the `x` vector.
...
We can derive the expression as follows,
...
Yes, log-likelihood seems to get the maximum value around MLE, which is expected because it is the maximum value of the curve.
...
$$
p\left( H \mid AB,\mu \right)=\binom{AB}{H} \mu^H(1-\mu)^{AB-H}\ \ \ \ \ \ \ \     (**)
$$
...
Figures in 3c) and 2f) are exactly the same because right hand side of $(*)$ in 3b) and the final expression in 2a) are the same.
...
The figure in 3e) is looking "similar" to the ones in 3c) and 2f) and they all give the same value for MLE. Since the binomial coefficient is involved in 3e), its graph has a scaling factor.
...
For the "uniform" set of shape parameters:
...
For the "uniform" set of shape parameters, the mean is:
...
Pdf for the "uniform" set of shape parameters is a constant function and thus ut can be stated that event probability values that are "ruled out" in that case. 
...
$$
p\left( m \mid N,\mu \right)=\binom{N}{m} \mu^m(1-\mu)^{N-m}\ \ \ \ \ \ \ \     (**)
$$
...
$$
p(\mu \mid a,b)=\text{Beta}(\mu \mid a,b)= \frac{\Gamma(a+b)}{\Gamma(a)\Gamma(b)}\mu^{(a-1)}(1-\mu)^{(b-1)}  \ \ \ \ \ \ \ \     (**)
$$
...
$$ 
p(\mu \mid m,N) \propto \text{Binomial}(m\mid N,\mu)p(\mu)\ \ \ \ \ \ \ \ \ \ (1)
$$
...
$$
$$ 
= (a+m-1)log[\mu]+(N-m+b-1)\log[1-\mu]\ \ \ \ \ \ \ \ (2)
$$
...
If we take the derivative of (2) in 5c), we get

$$ 
\frac{d}{d\mu}\log[p(\mu \mid m,N)] \propto \frac{a_{new}-1}{\mu}-\frac{b_{new}-1}{1-\mu}\ \ \ \ \ \ \ \ \ \ (1)
$$
...
When we set r.h.s. of expression (1) equal to 0 in 5d) above, we get
...
2) Middle 90% credible interval of the posterior belief associated with the uniform prior equals 0.5528245-0.2858488 = `r 0.5528245-0.2858488`.

3) Middle 90% credible interval of the posterior belief associated with the informative prior equals 0.4159531-0.2381307 = `r 0.4159531-0.2381307`. This value is less than the one above, which is expected because "informative" case contains more prior observations.
...
2) In this 2nd case, middle 90% credible interval of the posterior belief associated with the uniform prior equals 0.2616117-0.18117530 = `r 0.2616117-0.18117530`.

3) In this 2nd case, middle 90% credible interval of the posterior belief associated with the informative prior equals 0.1164100-0.06090699 = `r 0.1164100-0.06090699`. This value is less than the one above, which is expected because "informative" case contains more prior observations.
