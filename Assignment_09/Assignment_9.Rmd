...
Yes, in the plot below, it looks like all inputs have roughly the same scale.
...
All 75 figures look similar above and there do not seem to be a direct relationship (e.g. linear or quadratic) between inputs and output y. 
...
I think the inputs are not highly correlated because the plot in Problem 1d) shows mostly light-colored blocks.
...
As shown in the table below, 14 of the inputs are statistically significant.
...
As it can be seen in the plot above, the coefficient values are not close to each other. This is because with a high value of lambda, the Ridge regression coefficients should be considerably shrunken towards zero compared to the MLEs, as it is observed in the plot.
...
log_lambdagrid <- seq(log_lambdamin, log_lambdamax, length.out = 101)
lambda_grid <- exp(log_lambdagrid)
```
...
The inputs opposite nonzero values on the table below matter based on the tuned Lasso model.
...
As we see on the table above the best set of tuning parameters are alpha = 0.6092747 and lambda = 0.3129573. Since the alpha value is close to 1, then it suggests that the model is more consistent with Lasso regression.
...
The inputs opposite nonzero values on the table below are the non-zero coefficients.
...
As it can be seen in the plot in 5e) above, the most important input is "x07". Also, the variable importance rankings are consistent with the coefficient magnitudes we printed to the screen in Problem 5d). Foe examle in the table there the coefficient of "x07" is "-6.2683694211", which has the largest magnitude.
...
Yes, based on the visualizations on the figures below, the inputs are highly correlated.
...
There are 45 colums in the design matrix as expressed in the code chunk below.
...
2) Since the alpha value is close to 1, then it suggests that the model is more consistent with Lasso regression.
...
As it can be seen in the plot above, the most important input is “x4:x6”. Also, the rankings are consistent with the coefficient magnitudes we printed to the screen in Problem 6j). For examle in the table there the coefficient of “x4:x6” is “-1.3957406220”, which has the largest magnitude.
...
Yes, the output to input relationship appear to vary across the levels (categories) of `v` as it can be seen on the plot below.
